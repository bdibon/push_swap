...
/*                                                 #+#   #+    #+    #+#      */
/*   Created: 2018/08/25 13:58:11 by bodibon      #+#   ##    ##    #+#       */
/*   Updated: 2018/08/25 13:59:20 by bodibon     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void		split_stack_2_a(t_split *split, t_med *med, t_nstack *stack)
{
	t_stack_node *snode;
	...
	if (snode->data >= stack->med && enqueue(split->op, ft_strdup("pb")))
	{
		...
		split_stack_2(split, med, stack);
	}
	else if (snode->data < stack->med)
	{
		if (check_split_path_ra_2(stack) < check_split_path_rra_2(stack))
		{
...
void		split_stack_2(t_split *split, t_med *med, t_nstack *stack)
{
	if (stack->nb_infmed == 0 || stack->nb_supmed == 0)
		return ;
	if (stack == split->stack_a)
		split_stack_2_a(split, med, stack);
}
